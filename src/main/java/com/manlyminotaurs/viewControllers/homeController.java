...
//				pathList = Singleton.getInstance().pathfindingContext.getPath(startNode, endNode, new AStarStrategyI());
        ...
        if (useL2 == true) {
            tblDirections.getTreeItem(5).setExpanded(true);
        }
        if (useL1 == true) {
            tblDirections.getTreeItem(4).setExpanded(true);
        }
        if (use1 == true) {
            tblDirections.getTreeItem(3).setExpanded(true);
        }
        if (use2 == true) {
            tblDirections.getTreeItem(2).setExpanded(true);
        }
        if (use3 == true) {
            tblDirections.getTreeItem(1).setExpanded(true);
        }
...
//				pathList = Singleton.getInstance().pathfindingContext.getPath(startNode, endNode, new AStarStrategyI());
    ...
    public void updateIconZoom() {
        if(scrollGroup.getScaleX()<1.5) {
            for (ImageView icon : iconList) {
                icon.setScaleX(2 - scrollGroup.getScaleX());
                icon.setScaleY(2 - scrollGroup.getScaleY());
            }
        }
    }
...
// 		nodePane.getTransforms().set(1, rotationTransform);
        ...
        if(scrollGroup.getScaleX()<1.5) {
            nodePane.setScaleX(2 - scrollGroup.getScaleX());
            nodePane.setScaleY(2 - scrollGroup.getScaleY());
        }
    ...
    private void printStartName(MouseEvent mouseEvent) {
        ...
        Rotate rotationTransform = new Rotate(-scrollGroup.getRotate(), -29, 0);
        startName.getTransforms().clear();
        startName.getTransforms().add(rotationTransform);

        if(scrollGroup.getScaleX()<1.5) {
            startName.setScaleX(2 - scrollGroup.getScaleX());
            startName.setScaleY(2 - scrollGroup.getScaleY());
        }
    ...
    private void printEndName(MouseEvent mouseEvent) {
        ...
        if(scrollGroup.getScaleX()<1.5) {
            endName.setScaleX(2 - scrollGroup.getScaleX());
            endName.setScaleY(2 - scrollGroup.getScaleY());
        }
