    ...
    public Request genNewRequest(RequestType requestType, Node nodeAt, String message, String senderID, int priority) {
        ...
        switch (requestType) {
            case MedicalRequest:
                ...
                newReq = new MedicalRequest(dataModel.getNextRequestID(), "MedicalRequest", priority, false, false, LocalDateTime.now(),LocalDateTime.of(0,1,1,1,1), nodeAt.getNodeID(), messageID, "Password123");
            ...
            case JanitorialRequest:
                messageID = dataModel.getNextMessageID();
                newMsg = new Message(messageID, message, false, LocalDate.now(), "11", senderID);
                newReq = new JanitorialRequest(dataModel.getNextRequestID(), "JanitorialRequest", priority, false, false, LocalDateTime.now(), LocalDateTime.of(0,1,1,1,1), nodeAt.getNodeID(), messageID, "Password123");
